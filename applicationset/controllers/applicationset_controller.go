...
func (r *ApplicationSetReconciler) getAppsToSync(applicationSet argov1alpha1.ApplicationSet, appDependencyList [][]string, currentApplications []argov1alpha1.Application) map[string]bool {
	...
	for stepIndex := range appDependencyList {
		...
		for _, appName := range appDependencyList[stepIndex] {
			...
			if r.isApplicationDataPotentiallyStale(appName, appStatus) {
...
func (r *ApplicationSetReconciler) isApplicationDataPotentiallyStale(appName string, appStatus argov1alpha1.ApplicationSetApplicationStatus) bool {
	app := argov1alpha1.Application{}
	if err := r.Get(context.TODO(), types.NamespacedName{Name: appName}, &app); err != nil {
		if !apierrors.IsNotFound(err) {
			// There was an issue with retrieving application, assume its stale
			return true
		}
	}
